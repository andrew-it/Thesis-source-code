fun main(args: Array<String>) {
    ...
    val main_lex = Lexemes(
            ...
            EXPR_TYPES.RET_TYPE, ReturnType(STD_TYPES.INT)
    )
    val main_decl = EntryPoint(main_lex)

    // Class Shape
    val shape_move_func = FunctionDecl(Lexemes(
            EXPR_TYPES.RET_TYPE, ReturnType(STD_TYPES.VOID),
            EXPR_TYPES.FUNC_NAME, SymbolicSeq("moveBy"),
            EXPR_TYPES.SIGNATURE, Parameters(
            arrayOf(SignatureParam(VarName("x"), ReturnType(STD_TYPES.INT)),
                    SignatureParam(VarName("y"), ReturnType(STD_TYPES.INT)))),
    ...
    val class_lexemes = Lexemes(
            EXPR_TYPES.TYPE_ALIAS, SymbolicSeq("Shape"),
